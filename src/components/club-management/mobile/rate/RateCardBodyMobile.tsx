...
import RateCardMobile from '@/components/club-management/mobile/rate/RateCardMobile';

function RateCardBodyMobile() {
            ...
            {Object.values(selectedRateCard.rates).length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {Object.values(selectedRateCard.rates).map((rateDetails) => (
                  <RateCardMobile key={rateDetails.rateId} rateDetails={rateDetails} />
                ))}
              </div>
            )}
          </div>
...
export default RateCardBodyMobile;
