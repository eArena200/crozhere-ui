    ...
    <div className="border border-gray-300 rounded-lg p-4 bg-gray-50 shadow-sm">
      ...
      {costBreakup?.length ? (
        costBreakup.map((breakup, idx) => (
          <div key={idx} className="space-y-1 border border-gray-200 p-1 rounded-sm bg-white mb-2">
            <div className="flex justify-between text-sm border-b p-1">
              <span className="text-md font-medium text-gray-700">{breakup.category}</span>
            ...
            <div className="space-y-2">
              {breakup.details?.map((item, j) => (
                <div
                  key={j}
                  className="flex justify-between items-start text-sm bg-white p-1"
                >
                  <div>
                  ...
                  <div className="text-gray-900 font-medium text-xs">₹{item.amount}</div>
                </div>
              ))}
            </div>
          </div>
